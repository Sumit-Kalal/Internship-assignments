    ...
    for t in tasks:
        if t.electrician_id:
            notifications.append({
                "type": "task",
                "message": f"New task assigned: {t.task}"
            })

        if t.status and t.status.lower() == "completed":
            notifications.append({
                "type": "complete",
                "message": f"Task completed: {t.task}"
            })
    ...
    for j in jobs:
        if j.deadline:
            ...
            if days_left == 0:
                notifications.append({
                    "type": "deadline",
                    "message": f"Deadline is today for job: {j.title}"
                })
            elif 0 < days_left <= 2:
                notifications.append({
                    "type": "deadline",
                    "message": f"Job deadline approaching: {j.title} in {days_left} day(s)"
                })
